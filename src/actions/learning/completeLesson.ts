...
type CompleteLessonError = { error: string; code: string }

export async function completeLesson(
  lessonId: string
): Promise<CompleteLessonResult | CompleteLessonError> {
  try {
    const session = await requireAuth()

    if (session.user.role !== "CHILD") {
      return { error: "Tylko dziecko może ukończyć lekcję.", code: "FORBIDDEN" }
    }
    ...
    if (!lesson) {
      return { error: "Lekcja nie istnieje lub jest niedostępna.", code: "LESSON_NOT_FOUND" }
    }
    ...
    await prisma.$transaction(async (tx) => {
      if (existingProgress) {
        ...
        await tx.lessonProgress.create({
          data: { childId: child.id, lessonId, completed: true, xpEarned },
        })
      }
    ...
    console.error("[completeLesson]", { lessonId, err })
    return { error: "Wystąpił błąd podczas zapisywania postępu.", code: "INTERNAL_ERROR" }
  }
}
