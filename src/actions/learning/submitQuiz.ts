...
import { xpToLevel, quizXpReward } from "@/src/lib/learning/xp"
...
type QuizError = { error: string; code: string }
...
export async function submitQuiz(
  ...
  try {
    ...
    if (session.user.role !== "CHILD") {
      return { error: "Tylko dziecko może rozwiązywać quiz.", code: "FORBIDDEN" }
    }
    ...
    if (!child) {
      return { error: "Profil dziecka nie istnieje. Dokończ onboarding.", code: "CHILD_NOT_FOUND" }
    }
    ...
    if (!quiz) {
      return { error: "Quiz nie istnieje.", code: "QUIZ_NOT_FOUND" }
    }
    ...
    for (const question of quiz.questions) {
      ...
      if (Array.isArray(given)) {
        ...
        if ([...correctArr].sort().join(",") === [...given].sort().join(",")) correctCount++
      } else {
    ...
    const streakUpdate = computeStreakUpdate(child.lastStreakDate, child.streakDays, child.longestStreak)
    ...
    const [lessonsCompletedCount, quizzesCompletedCount] = await Promise.all([
      prisma.lessonProgress.count({ where: { childId: child.id, completed: true } }),
      prisma.quizAttempt.count({ where: { childId: child.id, passed: true } }),
    ])
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.childProfile.update({
        where: { id: child.id },
        data: { xp: finalTotalXp, level: finalLevel, lastActiveAt: new Date(), ...streakData },
      })
    ...
    console.error("[submitQuiz]", { quizId, err })
    return { error: "Wystąpił błąd podczas zapisywania wyników quizu.", code: "INTERNAL_ERROR" }
  }
}
